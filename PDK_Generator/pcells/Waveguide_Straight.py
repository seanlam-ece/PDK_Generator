...
class Waveguide_Straight(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("wg_length", self.TypeInt, "Waveguide Length", default = {length})     
    self.param("wg_width", self.TypeInt, "Waveguide width", default = {width}) 
...
#    print(path)
    ...
    text = Text ('Lumerical_INTERCONNECT_component={cm-name}', t)
    shape = shapes(LayerDevRecN).insert(text)
    shape.text_size = 0.1/dbu
