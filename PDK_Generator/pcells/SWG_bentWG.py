...
class SWG_bentWG(pya.PCellDeclarationHelper):
    ...
    def __init__(self):
      ...
      self.param("pitch", self.TypeDouble, "Period [um]", default = {period})
      self.param("w", self.TypeDouble, "Waveguide Width [um]", default = {waveguide-width})
      self.param("r", self.TypeDouble, "Radius [um]", default = {radius}) # minimum radius is 5 um in TE
      self.param("ff", self.TypeDouble, "Duty Cycle", default = {duty-cycle})
      self.param("angle", self.TypeDouble, "Angle", default = {angle})
      self.param("gap", self.TypeDouble, "Bus Gap [um]", default = {bus-gap})
      self.param("W_ratio", self.TypeDouble, "Lin/Lout", default = {Lin/Lout})
      self.param("doublebus", self.TypeBoolean, "Bus Type (Single=False, Double=True)", default = {bus-type})
  ...
  #    self.param("textl", self.TypeLayer, "Text Layer", default = TECHNOLOGY['{{Layer}Text}'], hidden = True)
    ...
    def produce_impl(self):
      ...
      for i in range(0,nSeg*2):
          ...
          if ORDER: 
            xo.append((r+w/2)*math.cos(j*theta1+jj*theta2)+deltaL*math.sin(j*theta1+jj*theta2))
            yo.append((r+w/2)*math.sin(j*theta1+jj*theta2)-deltaL*math.cos(j*theta1+jj*theta2))
            xo.append((r-w/2)*math.cos(j*theta1+jj*theta2)-deltaL*math.sin(j*theta1+jj*theta2))
            yo.append((r-w/2)*math.sin(j*theta1+jj*theta2)+deltaL*math.cos(j*theta1+jj*theta2))
            ORDER = not(ORDER)
          else:
