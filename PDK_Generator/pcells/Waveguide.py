...
class Waveguide(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("radius", self.TypeDouble, "Radius", default = {radius})
    self.param("width", self.TypeDouble, "Width", default = {width})
    self.param("adiab", self.TypeBoolean, "Adiabatic", default = {adiabatic})
    self.param("bezier", self.TypeDouble, "Bezier Parameter", default = {bezier})
    self.param("widths", self.TypeList, "Widths", default =  {widths})
    self.param("offsets", self.TypeList, "Offsets", default = {offsets})
    self.param("CML", self.TypeString, "Compact Model Library (CML)", default = "{orig-techname}")
    self.param("model", self.TypeString, "CML Model name", default = "{model}") 
    self.cellName="Waveguide"
    
    # declare layer params
    self.param("layers", self.TypeList, "Layers", default = ['{{Layer}Waveguide}'], hidden = True)
  ...
  def coerce_parameters_impl(self):
    ...
    print("{orig-techname}.Waveguide coerce parameters")
    ...
    for lr in range(0, len(self.layers)):
      ...
      if self.layout.layer(TECHNOLOGY['{{Layer}Waveguide}']) == layer:
        if width != wg_width:
          self.widths[lr] = self.width
      # check to make sure that the DevRec is bigger than the waveguide width
      if self.layout.layer(TECHNOLOGY['{{Layer}DevRec}']) == layer:
        if width < wg_width:
  ...
  def produce_impl(self):
    ...
    pts = path.get_points()
    LayerPinRecN = self.layout.layer(TECHNOLOGY['{{Layer}PinRec}'])
    ...
    LayerDevRecN = self.layout.layer(TECHNOLOGY['{{Layer}DevRec}'])
