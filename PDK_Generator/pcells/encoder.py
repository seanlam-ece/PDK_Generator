...
class encoder(pya.PCellDeclarationHelper):
  def __init__(self):
    ...
    self.param("N", self.TypeDouble, "Number of bits (N)", default = {num-bits})
    self.param("binary", self.TypeString, "identity (binary size N)", default = {binary-size})
    self.param("start_period", self.TypeDouble, "start period (microns)", default = {start-period})
    self.param("stop_period", self.TypeDouble, "stop period (microns)", default = {stop-period})
    self.param("corrugation_widths",self.TypeList,"Corrugations widths (microns)", default = {corrugation-widths})
    self.param("wg_width", self.TypeDouble, "Waveguide width", default = {wg-width})
    self.param("length", self.TypeInt, "length (microns)", default = {length})
    self.param("sum_format", self.TypeDouble, "sum format (1, 2, or 3)", default = {sum-format})
