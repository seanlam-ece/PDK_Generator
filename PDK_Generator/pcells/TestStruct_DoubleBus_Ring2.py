...
class TestStruct_DoubleBus_Ring2(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("npoints", self.TypeInt, "Number of points", default = {num-points})     
    self.param("textpolygon", self.TypeInt, "Draw text polygon label? 0/1", default = {text-polygon})
    
    # declare layer params
    self.param("silayer", self.TypeLayer, "Layer", default = TECHNOLOGY['{{Layer}Waveguide}'], hidden = True)
    self.param("textlayer", self.TypeLayer, "Text Layer", default = TECHNOLOGY['{{Layer}Text}'], hidden = True)
  ...
  def produce_impl(self):
    ...
    pcell = ly.create_cell("DoubleBus_Ring", "{libname}", {"r": r, "w": wg_width, "g": g, "silayer": LayerSi, "devrec": self.devrec, "pinrec": self.pinrec })
    print( "pcell: %s, %s" \
    ...
    if GC_imported == None:
      GC_imported = ly.create_cell(GC_name, "{libname}").cell_index()
    else:
