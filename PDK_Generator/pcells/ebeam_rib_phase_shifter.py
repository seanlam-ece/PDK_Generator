...
class ebeam_rib_phase_shifter(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("npp_distance", self.TypeDouble, "Npp Edge to Si Waveguide (um)", default = {npp-si-separation})
...
#    self.param("vc_dw", self.TypeDouble, "VC Offset", default = 5)
    ...
    self.param("vc_si_distance", self.TypeDouble, "VC Edge to Si Waveguide (um)", default = {vc-si-separation})
...
#    LayerNN = ly.layer(self.nlayer)
    ...
    if 1:
      ...
      pts = [Point(-l/2 - in_taper, -in_rib/2), Point( -l/2, -w/2), Point(-l/2, w/2), Point(-l/2 - in_taper, in_rib/2)]
      ...
      for i in range(0,N + 1):
        pts.append(Point(-l/2 - in_taper + in_taper/N * i, (in_slab-overlay_ebl)/2 + ((sw/2  - (in_slab-overlay_ebl))/(N**order))*(i**order)))
      for i in range(0,N + 1):
        pts.append(Point(-l/2 - in_taper + in_taper/N * (N - i), - (in_slab-overlay_ebl)/2 - ((sw/2  - (in_slab-overlay_ebl))/(N**order))*((N-i)**order)))
      ...
      for i in range(0,N + 1):
        pts.append(Point(l/2 + in_taper - in_taper/N * i, (in_slab-overlay_ebl)/2 + ((sw/2  - (in_slab-overlay_ebl))/(N**order))*(i**order)))
      for i in range(0,N + 1):
        pts.append(Point(l/2 + in_taper/N * i, - (in_slab-overlay_ebl)/2 - ((sw/2  - (in_slab-overlay_ebl))/(N**order))*((N-i)**order)))
      self.cell.shapes(LayerSlab).insert(Polygon(pts))
  
      from SiEPIC._globals import PIN_LENGTH as pin_length
  
      # Pin on the left side:
      p1 = [Point(- l/2 - in_taper + pin_length/2,0), Point(- l/2 - in_taper - pin_length/2,0)]
      ...
      p2 = [Point(l/2 + in_taper - pin_length/2,0), Point(l/2 + in_taper + pin_length/2,0)]
    ...
    for i in range(0, self.segments + 1):
        ...
        vc_temp_upper = pya.Box((l - contact_size)/self.segments *  i - l/2 + vc_to_npp_exclusion, w/2 + vc_si_distance,(l - contact_size)/self.segments *  i - l/2 + contact_size - vc_to_npp_exclusion, w/2 + vc_si_distance + contact_size)
        self.cell.shapes(LayerVCN).insert(vc_temp_upper)
        wg_temp_upper = pya.Box((l - contact_size)/self.segments *  i - l/2 - overlay, sw/2,(l - contact_size)/self.segments *  i - l/2 + contact_size + overlay, w/2 + vc_si_distance + contact_size + overlay)
        self.cell.shapes(LayerNPPN).insert(wg_temp_upper)
        wg_temp_upper = pya.Box((l - contact_size)/self.segments *  i - l/2-2*overlay, sw/2,(l - contact_size)/self.segments *  i - l/2 + contact_size+2*overlay, w/2 + vc_si_distance + contact_size+2*overlay)
        self.cell.shapes(LayerSlab).insert(wg_temp_upper)
        
        vc_temp_lower = pya.Box((l - contact_size)/self.segments *  i - l/2 + vc_to_npp_exclusion, -w/2 - vc_si_distance,(l - contact_size)/self.segments *  i - l/2 + contact_size - vc_to_npp_exclusion, -w/2 - vc_si_distance - contact_size)
        self.cell.shapes(LayerVCN).insert(vc_temp_lower)
        wg_temp_lower = pya.Box((l - contact_size)/self.segments *  i - l/2 - overlay , -sw/2,(l - contact_size)/self.segments *  i - l/2 + contact_size+ overlay, -(w/2 + vc_si_distance + contact_size + overlay))
        self.cell.shapes(LayerNPPN).insert(wg_temp_lower)
        wg_temp_lower = pya.Box((l - contact_size)/self.segments *  i - l/2-2*overlay, -sw/2,(l - contact_size)/self.segments *  i - l/2 + contact_size+2*overlay, -(w/2 + vc_si_distance + contact_size+2*overlay))
        self.cell.shapes(LayerSlab).insert(wg_temp_lower)

        metal_temp    = pya.Box((l - contact_size)/self.segments *  i - l/2 - overlay, -(w/2 + vc_si_distance + contact_size+1*overlay) - metal_routing_distance_to_node * (i%2),(l - contact_size)/self.segments *  i - l/2 + contact_size + overlay, w/2 + vc_si_distance + contact_size+1*overlay + metal_routing_distance_to_node * ((i+1)%2))
    ...
    dev = Box(-l/2-in_taper, -sw/2 - metal_routing_distance_to_node - contact_size - metal_routing_width,
    +l/2+in_taper, sw/2 + metal_routing_distance_to_node + contact_size + metal_routing_width)
