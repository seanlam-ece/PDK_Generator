...
class Bent_Contra_DC(pya.PCellDeclarationHelper):
  """
  ...
  def __init__(self):
    ...
    self.param("radius", self.TypeDouble, "Radius (um)", default = {radius})
    self.param("gap", self.TypeDouble, "Gap (um)", default = {gap})
    self.param("bus_width", self.TypeDouble, "Bus Width (um)", default = {bus-width})
    self.param("ring_width", self.TypeDouble, "Ring Width (um)", default = {ring-width})
    self.param("period", self.TypeDouble, "Gratings Period (nm)", default = {period})
    self.param("deltaWB", self.TypeDouble, "Bus Corrugation Width (um)", default = {bus-corrugation-width})
    self.param("deltaWR", self.TypeDouble, "Ring Corrugation Width (um)", default = {ring-corrugation-width})
    self.param("gamma", self.TypeDouble, "N (number of corrugations)", default = {num-corrugations})
    self.param("busBend", self.TypeDouble, "Bus-to-straight bend raidus", default = {bus-to-straight-bend-radius})
    
    # declare layer params
    self.param("silayer", self.TypeLayer, "Si Layer", default = TECHNOLOGY['{{Layer}Waveguide}'], hidden = True)
    self.param("silayer_gratings", self.TypeLayer, "Si Gratings Layer", default = TECHNOLOGY['{{Layer}Gratings Layer 6nm}'], hidden = True)
