...
class Ring_Filter_DB(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("vllayer", self.TypeLayer, "VL Layer", default = TECHNOLOGY['{{Layer}Via (VL)}'], hidden = True)
    self.param("mllayer", self.TypeLayer, "ML Layer", default = TECHNOLOGY['{{Layer}Metal (ML)}'], hidden = True)
    self.param("mhlayer", self.TypeLayer, "MH Layer", default = TECHNOLOGY['{{Layer}Heater}'], hidden = True)
...
#    TECHNOLOGY = get_technology_by_name('{orig-techname}')
    ...
    pcell = ly.create_cell("Ring", "{libname}", { "layer": LayerSi, "radius": self.r, "width": self.w } )
