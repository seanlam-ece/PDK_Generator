...
class SWG_to_strip_waveguide(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("wavelength", self.TypeDouble, "Wavelength", default = {wavelength})
    self.param("fishbone", self.TypeBoolean, "Fishbone", default = {fishbone})     
    self.param("length", self.TypeDouble, "Waveguide length", default = {waveguide-length})     
    self.param("taper_fraction", self.TypeDouble, "Strip taper length fraction (0 to 1)", default = {strip-taper-length-fraction})     
    self.param("period_strip", self.TypeDouble, "SWG Period at strip end (microns)", default = {swg-period-strip-end})     
    self.param("period_swg", self.TypeDouble, "SWG Period at SWG end (microns)", default = {swg-period-swg-end})     
    self.param("wg_width_strip", self.TypeDouble, "Strip Waveguide width at the taper end (microns)", default = {strip-waveguide-width-taper-end})     
#    self.param("wg_width_swg_taperend", self.TypeDouble, "Waveguide width at SWG taper end (microns)", default = 0.4)     
    self.param("wg_width_swg", self.TypeDouble, "SWG Waveguide at end (microns)", default = {swg-waveguide-width-end})     
    self.param("wg_width_taper", self.TypeDouble, "Nanotaper width at taper end (microns)", default = {nanotaper-width-taper-end})     
    self.param("duty_strip", self.TypeDouble, "SWG duty cycle at strip end (0 to 1)", default = {swg-duty-cycle-strip-end})     
    self.param("duty_swg", self.TypeDouble, "SWG duty cycle at SWG end (0 to 1)", default = {swg duty-cycle-swg-end})   
