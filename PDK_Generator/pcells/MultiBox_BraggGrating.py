...
class MultiBox_BraggGrating(pya.PCellDeclarationHelper):
    def __init__(self):
      ...
      self.param("pitch", self.TypeDouble, "Period of SWG [um]", default = {period-swg})
      self.param("w", self.TypeDouble, "Waveguide Width [um]", default = {waveguide-width})
      self.param("wc", self.TypeDouble, "Corrugation width [um]", default = {corrugation-width}) # minimum radius is 5 um in TE
      self.param("ff", self.TypeDouble, "Duty Cycle", default = {duty-cycle})
      #self.param("angle", self.TypeDouble, "Angle", default = 360)
      self.param("gap", self.TypeDouble, "Row Gap [um]", default = {row-gap})
      #self.param("gap2", self.TypeDouble, "Bus Gap [um]", default = 0.06)
      self.param("row", self.TypeDouble, "Number of Rows of main WG", default = {rows-main-wg})
      self.param("Length", self.TypeDouble, "Bragg Grating Length on Each side [um]", default = {length})
      self.param("taperL", self.TypeDouble, "Taper Length [um]", default = {taper-length})
      self.param("phaseshifted", self.TypeBoolean, "Bragg Type (Uniform=False, Phase-shifted=True)", default = {bragg-type})
