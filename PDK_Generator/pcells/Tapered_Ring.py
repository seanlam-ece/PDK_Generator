...
class Tapered_Ring(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("w_top", self.TypeDouble, "Top width", default = {top-width})
    self.param("w_bot", self.TypeDouble, "Bottom width (>Top Width)", default = {bottom-width})
    self.param("radius", self.TypeDouble, "Radius", default = {radius})
    
    # declare layer params
    self.param("LayerSi", self.TypeLayer, "Si Layer", default = TECHNOLOGY['{{Layer}Waveguide}'], hidden = True)
