...
class Contra_DC_SWG_segmented(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("grating_period", self.TypeDouble, "Sub-wavelength period (microns)", default = {swg-period})
    self.param("cdc_period", self.TypeDouble, "Pertrubation period (microns)", default = {perturbation-period})
    self.param("gap", self.TypeDouble, "Minimum gap (microns)", default = {min-gap})
    self.param("corrugation_width", self.TypeDouble, "Waveguide Corrugration width (microns)", default = {waveguide-corrugation-width})
