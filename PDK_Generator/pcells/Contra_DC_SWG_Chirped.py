...
class Contra_DC_SWG_Chirped(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("grating_period_start", self.TypeDouble, "Grating period start (microns)", default = {grating-period-start})
    self.param("grating_period_end", self.TypeDouble, "Grating period stop (microns)", default = {grating-period-end})
...
#    self.param("textl", self.TypeLayer, "Text Layer", default = TECHNOLOGY['{{Layer}Text}'], hidden = True)
  ...
  def display_text_impl(self):
    ...
    return "Contra_DC_SWG_chirped%s" % \
   ...
   # box_width = int(round(grating_period*self.duty))
    ...
    for i in range(0,N_boxes):
      ...
      if i%2 == True:
        ...
        box1_a = Box(x, -half_w1-deltaW1, x + box_width[i], half_w1-deltaW1)
        shapes(LayerSiN).insert(box1_a)
        
        box2_a = Box(x+grating_period[i], -half_w2-deltaW2, x + grating_period[i]+box_width[i], half_w2-deltaW2).transformed(t)
        ...
        box2_b = Box(x+grating_period[i], -half_w2, x +grating_period[i]+ box_width[i], half_w2).transformed(t)
        shapes(LayerSiN).insert(box2_b)
      
    # missing periods due to misalignments
    box_final = Box(x+grating_period[i-1], -half_w1, x +grating_period[i-1]+ box_width[i-1], half_w1)
    shapes(LayerSiN).insert(box_final)
    box_final = Box(-box_width[i-1]/2, -half_w2-deltaW2, box_width[i-1]/2, half_w2-deltaW2).transformed(t)
    shapes(LayerSiN).insert(box_final)
    ...
    t = Trans(Trans.R0, to_itype(-box_width[0]/2,dbu*1000),-deltaW1/2)
    ...
    t = Trans(Trans.R0, to_itype(-box_width[0]/2,dbu*1000),vertical_offset-deltaW2/2)
    pin = Path([Point(pin_length/2, 0), Point(-pin_length/2, 0)], w)
    pin_t = pin.transformed(t)
    shapes(LayerPinRecN).insert(pin_t)
    text = Text ("pin2", t)
    ...
    w = to_itype(self.wg2_width,dbu)
    t = Trans(Trans.R0, to_itype(x+grating_period[0]+box_width[0],dbu*1000),vertical_offset-deltaW2/2)
    pin = Path([Point(-pin_length/2, 0), Point(pin_length/2, 0)], w)
    pin_t = pin.transformed(t)
    shapes(LayerPinRecN).insert(pin_t)
    text = Text ("pin3", t)
    ...
    w = to_itype(self.wg1_width,dbu)
    t = Trans(Trans.R0, to_itype(x+grating_period[0]+box_width[0],dbu*1000),-deltaW1/2)
    pin = Path([Point(-pin_length/2, 0), Point(pin_length/2, 0)], w)
    pin_t = pin.transformed(t)
    shapes(LayerPinRecN).insert(pin_t)
    text = Text ("pin4", t)
