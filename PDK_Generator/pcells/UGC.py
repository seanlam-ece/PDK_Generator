...
class UGC(pya.PCellDeclarationHelper):
  """
  ...
  def __init__(self):
    ...
    self.param("wavelength", self.TypeDouble, "Design Wavelength (micron)", default = {design-wavelength})
    self.param("Si_thickness", self.TypeDouble, "Silicon Thickness (micron)", default = {wg-thickness})
    self.param("etch_depth", self.TypeDouble, "Etch Depth (micron)", default = {etch-depth})
    self.param("pol", self.TypeString, "Polarization", default = "{polarization}")
    self.param("n_t", self.TypeDouble, "Cladding Index", default = {cladding-index})
    self.param("angle_e", self.TypeDouble, "Taper Angle (deg)", default = {taper-angle})
    self.param("grating_length", self.TypeDouble, "Grating Length (micron)", default = {grating-length})
    self.param("taper_length", self.TypeDouble, "Taper Length (micron)", default = {taper-length})
    self.param("dc", self.TypeDouble, "Duty Cylce", default = {duty-cycle})
    self.param("t", self.TypeDouble, "Waveguide Width (micron)", default = {waveguide-width})
    self.param("theta_c", self.TypeDouble, "Insertion Angle (deg)", default = {insertion-angle})
...
#    return "temporary placeholder"
  ...
  def produce_impl(self):
    ...
    LayerTextN = get_technology()['{{Layer}Text}']
