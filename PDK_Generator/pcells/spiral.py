...
class spiral(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("length", self.TypeDouble, "Target Waveguide length", default = {waveguide-length})     
    self.param("wg_width", self.TypeDouble, "Waveguide width (microns)", default = {waveguide-width})     
    self.param("min_radius", self.TypeDouble, "Minimum radius (microns)", default = {min-radius})     
    self.param("wg_spacing", self.TypeDouble, "Waveguide spacing (microns)", default = {waveguide-spacing})     
    self.param("spiral_ports", self.TypeInt, "Ports on the same side? 0/1", default = {ports-same-side})
  ...
  def produce_impl(self):
    ...
    LayerTextN = ly.layer(get_technology()['{{Layer}Text}'])
