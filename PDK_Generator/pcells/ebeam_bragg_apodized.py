...
class ebeam_bragg_apodized(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("corrugation_width", self.TypeDouble, "Corrugration width (microns)", default = {corrugation-width})     
    self.param("misalignment", self.TypeDouble, "Grating misalignment (microns)", default = {misalignment})     
    self.param("sinusoidal", self.TypeBoolean, "Grating Type (Rectangular=False, Sinusoidal=True)", default = {grating-type})     
    self.param("wg_width", self.TypeDouble, "Waveguide width", default = {waveguide-width})     
