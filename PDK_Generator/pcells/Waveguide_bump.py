...
class Waveguide_bump(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("length", self.TypeDouble, "Regular Waveguide length", default = {waveguide-length})     
...
# for now, let the user specify the unknown theta 
    self.param("theta", self.TypeDouble, "Waveguide angle (degrees)", default = {waveguide-angle})     
    self.param("wg_width", self.TypeDouble, "Waveguide width (microns)", default = {waveguide-width}) 

    # declare layer params
    self.param("waveguide", self.TypeLayer, "Waveguide Layer", default = TECHNOLOGY['{{Layer}Waveguide}'], hidden = True)
    ...
    self.param("text", self.TypeLayer, "Text Layer", default = TECHNOLOGY['{{Layer}Text}'], hidden = True)
...
#    2*pi*r*(4*theta/360) = length + self.delta_length
    ...
    print("{orig-techname}: Waveguide_bump complete.")
